...
{
	MRT mrt = { float3(1, 1, 1), float3(1, 1, 1) };
	return mrt;
#if 0
...
# define SAMPLE_TEX(texName) texName.Sample(defaultSampler, vsOutput.uv)
	...
	float3 colorSum = 0;
	{
		...
		colorSum += ApplyAmbientLight(diffuseAlbedo, ao, g_globalCB.AmbientIntensity);
	}
	...
	colorSum += ApplyDirectionalLight(diffuseAlbedo, specularAlbedo, specularMask, gloss, normal, viewDir, g_globalCB.SunDirection, g_globalCB.SunIntensity, vsOutput.shadowCoord, texShadow);
