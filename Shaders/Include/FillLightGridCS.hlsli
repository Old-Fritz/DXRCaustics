...
StructuredBuffer<Lights::LightData> lightBuffer : register(t0);
	...
	uint GI : SV_GroupIndex)
{
	...
	for (uint lightIndex = GI; lightIndex < LightsCount; lightIndex += 64)
	{
		Lights::LightData lightData = lightBuffer[lightIndex];
		float3 lightWorldPos = lightData.m_pos;
		float lightCullRadius = sqrt(lightData.m_radiusSq);
