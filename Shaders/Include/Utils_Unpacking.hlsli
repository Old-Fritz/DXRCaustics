...
// unpacking
float4 Unpacking::UnPackUShort4N(PT_USHORT4N val)
{
	float4 res;
	res.x = val.x & 0xFFFF;
...
uint4 Unpacking::UnPackUByte4(PT_UBYTE4 val)
{
	uint4 res;
	res.x = val & 0xFF;
	res.y = (val >> 8) & 0xFF;
...
float4 Unpacking::UnPackUByte4N(PT_UBYTE4N val)
{
	uint4 res = UnPackUByte4(val);
	return res / 255.0;
}
float2 Unpacking::UnPackFloat16_2(PT_FLOAT16_2 val)
{
	float2 res;
	res.x = f16tof32(val);
...
float4 Unpacking::UnPackDec4(PT_DEC4 val)
{
	float4 res;
	res.x = val & 0x3FF;
	res.y = (val >> 10) & 0x3FF;
...
float4 Unpacking::UnPackColor(PT_COLOR val)
{
	return UnPackUByte4N(val);
}

PT_DEC4 Unpacking::PackDec4(float4 val)
{
